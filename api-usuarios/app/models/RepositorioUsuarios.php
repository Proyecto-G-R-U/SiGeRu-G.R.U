...
<<<<<<< HEAD
        ...
        $this->consulta($sql, [1, 'Carlos Rodríguez', 'admin@sigeru.uy',         $hash, 'administrador', null,            null, null]);
        $this->consulta($sql, [2, 'Marta Pérez',      'recoleccion@sigeru.uy',   $hash, 'operario',      'recoleccion',   1,    null]);
...
<<<<<<< HEAD
        ...
        if ($hashNuevo !== null && $hashNuevo !== '') {
            $stmt = $this->consulta(
                'UPDATE usuario SET nombre = ?, email = ?, password_hash = ?, rol = ?, especialidad = ?, instalacion_id = ? WHERE id = ?',
                [$u->getNombre(), $u->getEmail(), $hashNuevo, $u->getRol(), $esp, $inst, $u->getId()]
            );
        } else {
            $stmt = $this->consulta(
